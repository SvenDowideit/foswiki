...
sub set_up {
    ...
    my $topicObject = Foswiki::Store->create(
        address => {
            web   => $this->{users_web},
            topic => "GropeGroup",
            data  => { _text => "   * Set GROUP = ScumBag,WikiGuest\n" }
        }
    );
    $topicObject->save();
    ...
    $topicObject = Foswiki::Store->create(
        address => {
            web   => $this->{users_web},
            topic => "NestingGroup",
            data  => { _text => "   * Set GROUP = GropeGroup\n" }
        }
    );
    $topicObject->save();
    $topicObject = Foswiki::Store->create(
        address => {
            web   => $this->{users_web},
            topic => "GroupWithHiddenGroup",
            data  => { _text => "   * Set GROUP = HiddenGroup,WikiGuest\n" }
        }
    );
    $topicObject->save();
    $topicObject = Foswiki::Store->create(
...
"   * Set GROUP = ScumBag\n   * Set ALLOWTOPICVIEW = AdminUser\n"
    ...
    $topicObject = Foswiki::Store->create(
        address => {
            web   => $this->{users_web},
            topic => "HiddenUserGroup",
            data  => { _text => "   * Set GROUP = ScumBag,HidemeGood\n" }
        }
    );
    $topicObject->save();
    ...
    $topicObject =
      Foswiki::Store->create(
        address => { web => $this->{users_web}, topic => "HidemeGood" } );
