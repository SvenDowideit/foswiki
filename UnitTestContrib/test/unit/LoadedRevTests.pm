...
sub test_phantom_topic {
    ...
    my $topicObject = Foswiki::Store::create(
        address => { web => $this->{test_web}, topic => "PhantomTopic" } );
...
sub test_good_topic {
    my $this        = shift;
    my $topicObject = Foswiki::Store::create(
        address => { web   => $this->{test_web}, topic => "GoodTopic" },
        data    => { _text => 'Let there be light' }
    );
    ...
    $topicObject = Foswiki::Store::load(
        address => { web => $this->{test_web}, topic => "GoodTopic", rev => 0 }
    );
    $this->assert_equals( 1, $topicObject->getLoadedRev() );

    $topicObject = Foswiki::Store::load(
        address => { web => $this->{test_web}, topic => "GoodTopic", rev => 1 }
    );
    $this->assert_equals( 1, $topicObject->getLoadedRev() );
    $topicObject = Foswiki::Store::load(
        address => { web => $this->{test_web}, topic => "GoodTopic", rev => 2 }
    );
    $this->assert_equals( 1, $topicObject->getLoadedRev() );
}
...
sub test_borked_TOPICINFO_save {
    my $this        = shift;
    my $topicObject = Foswiki::Store::create(
        address => { web   => $this->{test_web}, topic => "BorkedTOPICINFO" },
        data    => { _text => <<SICK} );
...
WHEE
    ...
    my $topicObject = Foswiki::Store::load(
        address => { web => $this->{test_web}, topic => "NoCommaV" } );
    $this->assert_equals( 1, $topicObject->getLoadedRev() );
...
SICK
    ...
    $topicObject =
      Foswiki::Store::load( address =>
          { web => $this->{test_web}, topic => "BorkedTOPICINFO", rev => 1 } );
    $this->assert_equals( 1, $topicObject->getLoadedRev() );
    $this->assert_matches( qr/elderberries/, $topicObject->text() );

    $topicObject =
      Foswiki::Store::load( address =>
          { web => $this->{test_web}, topic => "BorkedTOPICINFO", rev => 2 } );
    $this->assert_equals( 2, $topicObject->getLoadedRev() );
    ...
    $topicObject =
      Foswiki::Store::load( address =>
          { web => $this->{test_web}, topic => "BorkedTOPICINFO", rev => 3 } );
    $this->assert_equals( 2, $topicObject->getLoadedRev() );
    $this->assert_matches( qr/lovely muck/, $topicObject->text() );

    $topicObject =
      Foswiki::Store::load( address =>
          { web => $this->{test_web}, topic => "BorkedTOPICINFO", rev => 0 } );
